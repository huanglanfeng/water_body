  ...
  <div class="table-container">
    ...
    <div class="top-bar">
      <el-form :inline="true" :model="searchForm">
        <el-form-item>
          <el-input v-model="searchForm.keyword" placeholder="请输入微生物名称/异常区域" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="paginatedData" stripe style="width: 100%">
      ...
      <el-table-column prop="name" label="微生物判断名称" />
...
import { organismget, organismdelete } from "@/api/index";
...
const handleExport = () => {
  ...
  exportToExcel(exportData, "微生物超标预警数据", "微生物超标");
  ElMessage({
    message: "导出成功",
    type: "success",
  });
  ...
  ElMessageBox.confirm("确定要删除该条记录么？", "提示", {
    ...
    .then(async () => {
      try {
        ...
        if (item) {
          await organismdelete({ params: { id: item.id, flag: "微生物" } });
        } else {
          ElMessage({
            message: "数据不存在",
            type: "error",
