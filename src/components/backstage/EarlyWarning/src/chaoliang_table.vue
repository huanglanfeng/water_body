  ...
  <div class="table-container">
    ...
    <div class="top-bar">
      <el-form :inline="true" :model="searchForm">
        <el-form-item>
          <el-input v-model="searchForm.keyword" placeholder="请输入设备编号/异常区域" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="paginatedData" stripe style="width: 100%" empty-text="暂无预警数据">
      ...
      <el-table-column prop="exceptionArea" label="堆积地区" />
...
import { garbageget, garbagedelete } from "@/api/index";
...
const handleExport = () => {
  ...
  const exportData = filteredData.value.map((item) => ({
    序号: item.id,
    设备编号: item.deviceId,
    堆积地区: item.exceptionArea,
    预警时间: item.warningTime,
  }));
  exportToExcel(exportData, "垃圾堆积预警数据", "垃圾堆积");
  ElMessage({
    message: "导出成功",
    type: "success",
  });
  ...
  ElMessageBox.confirm("确定要删除该条记录么？", "提示", {
    ...
    .then(async () => {
      try {
        ...
        if (item) {
          await garbagedelete({ params: { id: item.id, flag: "垃圾" } });
        } else {
          ElMessage({
            message: "数据不存在",
            type: "error",
