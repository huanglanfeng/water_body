  ...
  <div class="table-container">
    ...
    <div class="top-bar">
      <el-form :inline="true" :model="searchForm">
        <el-form-item>
          <el-input v-model="searchForm.keyword" placeholder="请输入放射元素/异常区域" clearable />
        </el-form-item>
        <el-form-item>
...
import { radiationget, radiationdelete } from "@/api/index";
...
const filteredData = computed(() => {
  ...
  return tableData.value.filter((item: form) => {
    return (
      (item.radioactiveElement && item.radioactiveElement.toLowerCase().includes(keyword)) ||
...
const handleExport = () => {
  ...
  const exportData = filteredData.value.map((item) => ({
    序号: item.id,
    放射元素: item.radioactiveElement,
    异常区域: item.exceptionArea,
    预警时间: item.warningTime,
  }));
  exportToExcel(exportData, "放射性元素过量预警数据", "放射性元素过量");
  ElMessage({
    message: "导出成功",
    type: "success",
  });
  ...
  ElMessageBox.confirm("确定要删除该条记录么？", "提示", {
    ...
    .then(async () => {
      try {
        ...
        if (item) {
          await radiationdelete({ params: { id: item.id, flag: "放射" } });
        } else {
          ElMessage({
            message: "数据不存在",
            type: "error",
