<template>
  <el-table
    ...
    style="width: 100%"
  >
  <el-table-column prop="site" label="地区" />
    <el-table-column prop="phos" label="总磷" />
    <el-table-column prop="sulfur" label="总硫" />
    <el-table-column prop="nitrogen" label="总氮" />
    <el-table-column prop="dissolvedOxygen" label="融氧量" />
    <el-table-column prop="detactTime" label="检测时间" />
...
import { ref } from "vue";
import { waterGet,waterDelete } from "@/api/index";
import type { form } from "./type";
import { ElMessage, ElMessageBox } from "element-plus";

const getData = async () => {
  let res = await waterGet({params:{id:2}});
  ...
  ElMessageBox.confirm("确定要删除该条记录么？", "提示", {
    ...
    .then(async () => {
      try {
        ...
        if (item) {
          await waterDelete({ params: { site: item.site, flag: "水溶物" } });
        }
